...
class PmdProfileImporterTest {
    ...
    void should_import_simple_profile() {
        Reader reader = read("/org/sonar/plugins/p3c/pmd/simple.xml");

        RulesProfile profile = importer.importProfile(reader, messages);

        assertThat(profile.getActiveRules()).hasSize(3);
        assertThat(profile.getActiveRuleByConfigKey("p3c-pmd", "rulesets/java/coupling.xml/ExcessiveImports")).isNotNull();
        assertThat(profile.getActiveRuleByConfigKey("p3c-pmd", "rulesets/java/design.xml/UseNotifyAllInsteadOfNotify")).isNotNull();
        assertThat(messages.hasErrors()).isFalse();
    }

    @Test
    void should_import_profile_with_xpath_rule() {
        Reader reader = read("/org/sonar/plugins/p3c/pmd/export_xpath_rules.xml");
    ...
    void should_import_parameter() {
        ...
        RulesProfile profile = importer.importProfile(reader, messages);
        ActiveRule activeRule = profile.getActiveRuleByConfigKey("p3c-pmd", "rulesets/java/coupling.xml/ExcessiveImports");
    ...
    void should_import_priority() {
        ...
        ActiveRule activeRule = profile.getActiveRuleByConfigKey("p3c-pmd", "rulesets/java/design.xml/UseNotifyAllInsteadOfNotify");
        assertThat(activeRule.getSeverity()).isSameAs(RulePriority.MINOR);

        activeRule = profile.getActiveRuleByConfigKey("p3c-pmd", "rulesets/java/coupling.xml/CouplingBetweenObjects");
        assertThat(activeRule.getSeverity()).isSameAs(RulePriority.CRITICAL);
    }

    @Test
    void should_import_pmd_configuration_with_unknown_nodes() {
        Reader reader = read("/org/sonar/plugins/p3c/pmd/complex-with-unknown-nodes.xml");
    ...
    void should_deal_with_unsupported_property() {
        ...
        RulesProfile profile = importer.importProfile(reader, messages);
        ActiveRule check = profile.getActiveRuleByConfigKey("p3c-pmd", "rulesets/java/coupling.xml/CouplingBetweenObjects");
